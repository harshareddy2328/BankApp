...
var doctorid = null;
var doctorname = null;
var baseUrl = "http://juno.clinic:4002";

var userContext = {identity: null, endpoint: null};

function init() {
    var identity = doctorid;
    if (!identity) {
        return;
    }

    userContext.identity = identity;
    logIn(identity, doctorname);

}

$(document).ready(function () {
//    chatinfo = JSON.parse($.cookie("chatinfo"));
    doctorid = $("#doctorid").data("docid");
    doctorname = $("#doctorname").data("docname");
...
function logIn(identity, displayName) {
    ...
    fingerprint.get(function (endpointId) {
        ...
        request(baseUrl + '/getToken?identity=' + identity + '&endpointId=' + endpointId, function (err, res) {
            if (err) {
...
function addInvitedChannel(channel) {
    console.log("I am invited channel. :)");
    console.log(channel);
    ...
    var $img = $('<img/>')
            .attr('src', 'http://gravatar.com/avatar/' + MD5(channel.sid) + '?s=30&d=mm&r=g')
            .appendTo($el);
    
    var $title = $('<span class="invited blink"/>')
            .text(channel.friendlyName)
            .appendTo($el);
...
//            }).appendTo($el);
    ...
    var $img = $('<img/>')
            .attr('src', 'http://gravatar.com/avatar/' + MD5(message.author) + '?s=60&d=mm&r=g')
            .appendTo($el);

    var author = "";
    
    if (doctorid == message.author) {
        author = "Dr. " + doctorname;
    } else {
        author = chatinfo.name + " (" + chatinfo.mobile + ")";
    }
...
function addMessage(message) {
    ...
    var $el = $('<li/>').attr('data-index', message.index).attr('class', classname);
    createMessage(message, $el);
...
function updateMembers() {
    ...
    activeChannel.getMembers()
            .then(members => members
                        ...
                        .sort(function (a, b) {
                            return a.getUser().then(user => user.online) < b.getUser().then(user => user.online);
                        })
                        .forEach(addMember));
...
function setActiveChannel(channel) {
    ...
    activeChannel.getAttributes().then(function (attributes) {
        ...
        if (attributes.mobile !== undefined)
            str += (" | " + attributes.mobile);

        if (attributes.description !== undefined)
            str += (" | " + attributes.description);

        $('#channel-desc').text(str);
    });
